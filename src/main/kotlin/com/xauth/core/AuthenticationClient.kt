...
class AuthenticationClient(userPoolId: String) : BaseClient(userPoolId) {
     ...
     * 检查当前登录状态
     */
    fun checkLoginStatus(checkLoginStatusParam: CheckLoginStatusParam): Call<CheckLoginStatusResponse> {
        checkAccessToken()
     ...
     *  创建用户
     */
    fun createUser(param: CreateUserParam): Call<CreateUserResponse> {

        return createGraphQLCall(param.createRequest(), object : TypeToken<GraphQLResponse<CreateUserResponse>>() {})
    }
}